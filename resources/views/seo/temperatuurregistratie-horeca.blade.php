...
<head>
    @php
        ...
        $faqItems = [
            ['Wat is temperatuurregistratie in de horeca?', 'Temperatuurregistratie is het vastleggen van temperaturen van koelingen, vriezers en producten om voedselveiligheid te waarborgen.'],
            ['Kan ik temperatuurregistraties digitaal uitvoeren?', 'Ja. Met TaskCheck registreer je temperaturen via smartphone, tablet of computer.'],
            ['Kan ik foto\'s toevoegen aan registraties?', 'Ja. Je kunt foto\'s, opmerkingen en bewijs toevoegen aan iedere controle.'],
...
<section class="relative overflow-hidden bg-white pt-24 pb-14 sm:pt-28 sm:pb-16">
    <div class="absolute inset-0 pointer-events-none" aria-hidden="true">
        <svg class="absolute inset-0 h-full w-full opacity-[.03]" xmlns="http://www.w3.org/2000/svg">
            <defs>
                <pattern id="seo-temp-horeca-dots" x="0" y="0" width="24" height="24" patternUnits="userSpaceOnUse">
                    <circle cx="1" cy="1" r="1.2" fill="#334155"/>
                </pattern>
            </defs>
            <rect width="100%" height="100%" fill="url(#seo-temp-horeca-dots)"/>
        </svg>
        <div class="absolute max-md:-right-[280px] max-md:top-[-200px] md:-right-[200px] md:-top-[300px] h-[min(520px,120vw)] w-[min(520px,120vw)] md:h-[800px] md:w-[800px] rounded-full bg-[radial-gradient(circle,rgba(14,165,233,.12)_0%,transparent_65%)]"></div>
    </div>
        ...
        <div class="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
            <div class="min-w-0">
                ...
                <h1 class="text-3xl font-extrabold leading-[1.08] tracking-tight text-slate-900 sm:text-5xl xl:text-[3.25rem]">
                    Temperatuurregistratie Horeca
                    <span class="relative inline-block">
                        <span style="background:linear-gradient(135deg,#0284c7,#06b6d4);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text">Digitaal Vastleggen</span>
                        <svg class="absolute -bottom-1 left-0 w-full" viewBox="0 0 300 8" preserveAspectRatio="none" style="height:5px" aria-hidden="true">
                            <path d="M1 6 C75 1, 225 1, 299 6" stroke="url(#seo-temp-horeca-ul)" stroke-width="3" stroke-linecap="round" fill="none"/>
                            <defs>
                                <linearGradient id="seo-temp-horeca-ul" x1="0" y1="0" x2="300" y2="0">
                ...
                <p class="mt-5 max-w-xl text-base leading-relaxed text-slate-500 sm:mt-6 sm:text-lg">
                    Temperatuurregistratie is een belangrijk onderdeel van voedselveiligheid binnen de horeca. Door temperaturen van koelingen, vriezers en producten regelmatig te controleren, verklein je de kans op bederf en voedselveiligheidsproblemen.
                </p>
                <p class="mt-3 max-w-xl text-base leading-relaxed text-slate-500">
                    Met TaskCheck registreer je temperaturen digitaal en heb je altijd inzicht in uitgevoerde controles.
                </p>
                ...
                <div class="mt-5 flex flex-wrap gap-x-5 gap-y-2 sm:mt-6 sm:gap-x-6">
                    @foreach(['Geen creditcard nodig','14 dagen gratis proberen','Afwijkingen direct zichtbaar'] as $b)
                    <span class="flex items-center gap-1.5 text-xs text-slate-500">
                        <svg class="h-3.5 w-3.5 shrink-0 text-cyan-500" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg>
                        {{ $b }}
                    </span>
                    @endforeach
                </div>
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="grid items-start gap-16 lg:grid-cols-2">
            <div>
                ...
                <p class="mt-3 font-medium text-slate-800">Daarom is het belangrijk om temperaturen regelmatig te controleren en vast te leggen.</p>
            </div>
            <div class="space-y-3">
                @foreach([
                    'Voedselverspilling',
...
<section class="border-b border-slate-100 bg-white py-20 sm:py-24">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="mt-12 grid gap-6 lg:grid-cols-3">
            @php
            $categories = [
                ['title' => 'Koelingen', 'items' => ['Koelingen', 'Drankkoeling', 'Werkbankkoeling', 'Koelcel', 'Saladette', 'Dessertkoeling'], 'color' => '#0284c7'],
                ['title' => 'Vriezers', 'items' => ['Vrieskast', 'Vriescel', 'Opslagvriezer'], 'color' => '#0891b2'],
                ['title' => 'Producten', 'items' => ['Warme gerechten', 'Koude gerechten', 'Buffetten', 'Bereide producten'], 'color' => '#06b6d4'],
            ];
            @endphp
            @foreach($categories as $cat)
            <div class="feature-card rounded-2xl border border-slate-200 bg-slate-50 p-6">
                <div class="mb-4 flex items-center gap-3">
                    <span class="flex h-9 w-9 items-center justify-center rounded-lg text-sm font-bold text-white" style="background:{{ $cat['color'] }}">
                        <svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M9.75 3.104v5.714a2.25 2.25 0 0 1-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 0 1 4.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M14.25 3.104v5.714a2.25 2.25 0 0 0 .659 1.591M5 14.5a2.25 2.25 0 0 0-2.25 2.219V17.5A2.25 2.25 0 0 0 5 19.5h14a2.25 2.25 0 0 0 2.25-2.25v-1.781a2.25 2.25 0 0 0-2.25-2.25M5 14.5l2.25-2.25M19.8 15.3 14.25 12.75 14.25 9.336"/></svg>
                    </span>
                    <h3 class="text-lg font-bold text-slate-900">{{ $cat['title'] }}</h3>
                </div>
                <ul class="space-y-2">
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="grid items-center gap-16 lg:grid-cols-2">
            <div>
                ...
                <p class="mt-4 text-sm font-semibold text-slate-700">Per controle kunnen zij:</p>
                <ul class="mt-4 space-y-3">
                    @foreach(['Temperatuur invoeren', 'Opmerkingen toevoegen', 'Foto\'s toevoegen', 'Afwijkingen melden'] as $action)
                    <li class="flex items-center gap-3 text-sm text-slate-700">
                        <svg class="h-5 w-5 shrink-0 text-cyan-500" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg>
                        {{ $action }}
                    </li>
                    @endforeach
                </ul>
            ...
            <div class="rounded-2xl border border-sky-200 bg-white p-6 shadow-sm">
                <p class="text-sm font-bold uppercase tracking-wider text-sky-600">Normen</p>
                <h3 class="mt-2 text-xl font-bold text-slate-900">Automatische signalering van afwijkingen</h3>
                <p class="mt-2 text-sm text-slate-500">Met TaskCheck kun je normen instellen. Wanneer een temperatuur buiten de norm valt, wordt dit direct zichtbaar zodat actie kan worden ondernomen.</p>
                <div class="mt-6 space-y-3">
                    @foreach([
                        ['Koelingen', 'Maximaal 7°C', 'bg-sky-50 border-sky-100'],
                        ['Vriezers', 'Minimaal -18°C', 'bg-cyan-50 border-cyan-100'],
                        ['Warmhouden', 'Minimaal 60°C', 'bg-amber-50 border-amber-100'],
                    ] as [$label, $norm, $style])
                    <div class="flex items-center justify-between rounded-xl border px-4 py-3 {{ $style }}">
                        <span class="text-sm font-semibold text-slate-800">{{ $label }}</span>
                        <span class="text-sm font-bold text-sky-700">{{ $norm }}</span>
                    </div>
                    @endforeach
                </div>
            </div>
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="grid items-center gap-16 lg:grid-cols-2">
            <div class="order-2 lg:order-1 grid gap-3 sm:grid-cols-2">
                @foreach([
                    ['Uitgevoerd', 'Welke registraties zijn uitgevoerd', 'text-emerald-600', 'bg-emerald-50'],
                    ['Ontbrekend', 'Welke controles ontbreken', 'text-amber-600', 'bg-amber-50'],
                    ['Afwijking', 'Welke temperaturen buiten de norm vallen', 'text-red-600', 'bg-red-50'],
                    ['Locaties', 'Welke locaties extra aandacht nodig hebben', 'text-sky-600', 'bg-sky-50'],
        ...
        <div class="grid items-center gap-16 lg:grid-cols-2">
            <div>
                ...
                <ul class="mt-6 space-y-3">
                    @foreach(['Overzicht per locatie', 'Centrale rapportages', 'Standaard werkwijze', 'Minder fouten'] as $benefit)
                    <li class="flex items-center gap-3 text-sm text-slate-700">
                        <svg class="h-5 w-5 shrink-0 text-sky-600" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg>
                        {{ $benefit }}
                    </li>
                    @endforeach
                </ul>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
                <p class="text-sm font-bold uppercase tracking-wider text-red-600">Papier</p>
                <h3 class="mt-2 text-xl font-bold text-slate-900">Stop met papieren temperatuurformulieren</h3>
                <p class="mt-2 text-sm text-slate-500">Veel horecabedrijven gebruiken nog steeds papieren registratielijsten. Met TaskCheck wordt alles digitaal opgeslagen en direct beschikbaar.</p>
                <ul class="mt-5 space-y-2">
                    @foreach(['Formulieren raken kwijt', 'Handschrift is onleesbaar', 'Geen realtime inzicht', 'Tijdrovend terugzoeken'] as $con)
                    <li class="flex items-start gap-2 text-sm text-slate-600">
                        <svg class="mt-0.5 h-4 w-4 shrink-0 text-red-400" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12"/></svg>
                        {{ $con }}
                    </li>
                    @endforeach
                </ul>
        ...
        <div class="grid gap-12 lg:grid-cols-2">
            <div>
                ...
                <div class="mt-8 grid gap-2 sm:grid-cols-2">
                    ...
                    <div class="flex items-center gap-2 text-sm text-slate-700">
                        <svg class="h-4 w-4 shrink-0 text-cyan-500" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg>
                        {{ $benefit }}
                    </div>
                    @endforeach
                </div>
            ...
            <div>
                ...
                <div class="mt-6 flex flex-wrap gap-2">
                    ...
                    <span class="rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-medium text-slate-700">{{ $target }}</span>
                    @endforeach
                </div>
            </div>
        </div>
