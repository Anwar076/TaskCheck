...
<div class="mx-auto max-w-4xl space-y-6">
    ...
    <form method="POST" action="{{ route('super-admin.subscriptions.store') }}" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">@csrf
        <div class="grid gap-5 sm:grid-cols-2">
            ...
            <div><label class="mb-1.5 block text-sm font-medium text-slate-700">Wanneer wordt er betaald?</label><select name="billing_period" class="w-full rounded-xl border-slate-300 text-sm">@foreach(\App\Models\Organisation\Company::BILLING_PERIODS as $periodKey => $period)<option value="{{ $periodKey }}" @selected(old('billing_period', 'monthly') === $periodKey)>{{ $period['label'] }}</option>@endforeach</select></div>
            <div><label class="mb-1.5 block text-sm font-medium text-slate-700">Prijs per betaalmoment excl. btw</label><input name="price" type="number" min="0" step="0.01" required value="{{ old('price') }}" class="w-full rounded-xl border-slate-300 text-sm"><p class="mt-1 text-xs text-slate-500">Dit volledige bedrag wordt bij ieder gekozen betaalmoment via Mollie geïncasseerd.</p></div>
