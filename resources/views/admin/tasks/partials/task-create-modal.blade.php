...
<div id="task-create-modal"
    ...
    <div class="absolute inset-x-0 bottom-0 flex max-h-[min(92dvh,92vh)] flex-col rounded-t-2xl border border-slate-200 bg-white shadow-2xl sm:inset-auto sm:left-1/2 sm:top-1/2 sm:max-h-[85vh] sm:w-full sm:max-w-lg sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-2xl md:max-w-xl lg:max-w-2xl">
        <form id="task-create-form" class="flex min-h-0 flex-1 flex-col">
            <div class="flex shrink-0 flex-col border-b border-slate-100">
                <div class="flex justify-center pt-2 sm:hidden" aria-hidden="true">
                    <span class="h-1.5 w-10 rounded-full bg-slate-300"></span>
                </div>
                <div class="flex items-start justify-between gap-3 px-4 py-3 sm:px-5 sm:py-4">
                    <div class="min-w-0 flex-1">
                        <h3 id="task-create-modal-title" class="text-base sm:text-lg font-semibold text-slate-900">Nieuwe taak</h3>
                        <p class="mt-0.5 truncate text-xs text-slate-500">In lijst: {{ $list->title }}</p>
                    </div>
                    <button type="button" data-task-create-close class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-slate-400 hover:bg-slate-100 hover:text-slate-600" aria-label="Sluiten">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" id="task-create-title" required autocomplete="off"
                               class="block w-full min-h-[44px] rounded-xl border border-slate-200 px-3 py-2.5 text-base sm:text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                    ...
                    <div>
                        ...
                        <select id="task-create-proof" class="block w-full min-h-[44px] rounded-xl border border-slate-200 px-3 py-2.5 text-base sm:text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20">
                    ...
                    <button type="button" id="task-create-expand-toggle"
                            class="flex w-full min-h-[44px] items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-left text-sm font-medium text-slate-700 hover:bg-slate-100">
                    ...
                    <div id="task-create-expanded" class="hidden space-y-4 border-t border-slate-100 pt-4">
                        ...
                        <div>
                            ...
                            <input type="text" id="task-create-norm-reference" maxlength="255" placeholder="Bijv. Werkprotocol §4.3 of ISO-clausule 8.5" class="block w-full min-h-[44px] rounded-xl border border-slate-200 px-3 py-2.5 text-base sm:text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20">
                        ...
                        <div>
                            ...
                            <textarea id="task-create-acceptance-criteria" rows="2" maxlength="2000" placeholder="Bijv. koelcel maximaal 7 °C, werkblad zichtbaar schoon of nooduitgang volledig vrij" class="block w-full min-h-[44px] rounded-xl border border-slate-200 px-3 py-2.5 text-base sm:text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20"></textarea>
                        ...
                        @if($showDayPicker)
                            <div>
                                <p class="mb-1 text-xs font-medium text-slate-600">Alleen op deze dagen</p>
                                <p class="mb-2 text-[11px] text-slate-400">Leeg = elke actieve dag</p>
                                <div class="grid grid-cols-4 sm:grid-cols-7 gap-2">
                                    @foreach($weekdayShort as $dayKey => $dayLabel)
                                        @php $available = in_array($dayKey, $listAvailableDays); @endphp
                                        <label class="flex min-h-[44px] flex-col items-center justify-center rounded-lg border px-1 py-2 text-center text-xs font-medium transition-colors
                        ...
                        <label class="flex items-center gap-3 cursor-pointer min-h-[44px]">
                            <input type="checkbox" id="task-create-signature" class="h-5 w-5 shrink-0 rounded border-slate-300 text-blue-600 focus:ring-blue-500">
                        ...
                        <div class="rounded-xl border border-slate-200 p-3 sm:p-4">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                <select id="task-create-metric-type" class="w-full min-h-[44px] rounded-lg border border-slate-200 px-3 py-2 text-base sm:text-sm">
                                    <option value="">Geen meting</option>
                                    <option value="temperature">Temperatuur</option>
                                    <option value="ph">pH</option>
                                </select>
                                <input type="text" id="task-create-metric-unit" placeholder="Eenheid" class="w-full min-h-[44px] rounded-lg border border-slate-200 px-3 py-2 text-base sm:text-sm">
                                <input type="number" step="0.1" id="task-create-metric-min" placeholder="Min" class="w-full min-h-[44px] rounded-lg border border-slate-200 px-3 py-2 text-base sm:text-sm">
                                <input type="number" step="0.1" id="task-create-metric-max" placeholder="Max" class="w-full min-h-[44px] rounded-lg border border-slate-200 px-3 py-2 text-base sm:text-sm">
                            </div>
                        </div>
                        ...
                        <div>
                            <div class="mb-2 flex items-center justify-between gap-2">
                                ...
                                <button type="button" id="task-create-add-checklist" class="inline-flex min-h-[36px] items-center rounded-lg px-2 text-xs font-medium text-blue-600 hover:bg-blue-50 hover:text-blue-800">+ Item</button>
                            </div>
                            <div id="task-create-checklist" class="space-y-2"></div>
                        </div>
                    </div>

                    <p id="task-create-error" class="hidden rounded-lg bg-red-50 px-3 py-2 text-xs text-red-700 break-words"></p>
                </div>
            </div>

            <div class="flex shrink-0 flex-col-reverse gap-2 border-t border-slate-100 px-4 py-3 sm:flex-row sm:items-center sm:justify-end sm:px-5 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
                <button type="button" data-task-create-close class="inline-flex min-h-[44px] w-full items-center justify-center rounded-xl px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 sm:w-auto">Annuleren</button>
                <button type="submit" id="task-create-submit" class="inline-flex min-h-[44px] w-full items-center justify-center rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-50 sm:w-auto">
                    Taak toevoegen
                </button>
            </div>
        </form>
