...
@section('breadcrumbs')
    ...
    <a href="{{ route('admin.users.show', $user) }}" class="text-slate-500 hover:text-slate-700 font-medium transition-colors truncate">{{ $user->name }}</a>
    <span class="text-slate-400">/</span>
                        ...
                        <div>
                            ...
                            <select name="location_id" id="location_id"
                                ...
                                @foreach($locations as $location)
                                    <option value="{{ $location->id }}" {{ (string) old('location_id', $user->location_id) === (string) $location->id ? 'selected' : '' }}>
                                        {{ $location->name }}{{ $location->is_active ? '' : ' (gearchiveerd)' }}
                                    </option>
                                @endforeach
                            </select>
                            @error('location_id')
