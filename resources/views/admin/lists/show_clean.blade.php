...
function toggleAssignmentType() {
    ...
    if (assignmentTypeRadio.value === 'user') {
        if (userAssignment) userAssignment.classList.remove('hidden');
        if (departmentAssignment) departmentAssignment.classList.add('hidden');
        if (departmentSelect) {
    ...
    } else if (assignmentTypeRadio.value === 'department') {
        if (userAssignment) userAssignment.classList.add('hidden');
        if (departmentAssignment) departmentAssignment.classList.remove('hidden');
        if (userSelect) {
            userSelect.value = '';
...
function removeAssignment(assignmentId) {
    ...
    if (assignmentType === 'user') {
        console.log('👤 Switching to user assignment mode');
        userAssignment?.classList.remove('hidden');
        departmentAssignment?.classList.add('hidden');
        if (departmentSelect) {
            departmentSelect.value = '';
            departmentSelect.removeAttribute('required');
            departmentSelect.disabled = true; // Disable the field completely
        }
        if (userSelect) {
        ...
        console.log('🏢 Switching to department assignment mode');
        userAssignment?.classList.add('hidden');
        departmentAssignment?.classList.remove('hidden');
        if (userSelect) {
            userSelect.value = '';
            userSelect.removeAttribute('required');
            userSelect.disabled = true; // Disable the field completely
        }
        if (departmentSelect) {
...
// No longer needed - using new agenda system
...
window.removeAssignment = function removeAssignment(assignmentId) {
    console.log('🗑️ Removing assignment with ID:', assignmentId);
    
    if (!assignmentId || assignmentId === '' || assignmentId === 'undefined') {
        console.error('❌ Invalid assignment ID:', assignmentId);
        alert('Ongeldige assignment ID. Kan niet verwijderen.');
        return;
    }
    ...
    if (confirm('Weet je zeker dat je deze toewijzing wilt verwijderen?')) {
        ...
        if (!csrfTokenMeta) {
            ...
            alert('CSRF token niet gevonden. Herlaad de pagina en probeer opnieuw.');
            return;
        }
        
        const csrfTokenValue = csrfTokenMeta.getAttribute('content');
        console.log('🔐 CSRF token for delete:', csrfTokenValue.substring(0, 10) + '...');
        ...
        const deleteUrl = `/admin/assignments/${assignmentId}`;
        ...
        .then(response => {
            ...
            if (response.ok || response.redirected) {
                console.log('✅ Assignment deleted successfully, refreshing page...');
                showSuccessToast('Toewijzing succesvol verwijderd! 🗑️');
                // Small delay to show feedback, then refresh
                setTimeout(() => {
                    window.location.reload();
                }, 800);
            } else {
                console.error('❌ Delete failed with status:', response.status);
                alert('Er is een fout opgetreden bij het verwijderen. Probeer opnieuw.');
        ...
        .catch(error => {
            ...
            alert('Er is een netwerkfout opgetreden. Probeer opnieuw.');
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Page loaded, initializing assignment functionality');
