        ...
        <div class="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div class="divide-y divide-gray-100">
                @foreach($submission->submissionTasks as $index => $submissionTask)
                    <div class="p-8 hover:bg-gray-50 transition-colors duration-200 submission-task" data-submission-task-id="{{ $submissionTask->id }}">
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                ...
                                <div class="flex items-start space-x-4 mb-6">
                                    ...
                                    <div class="flex-1">
                                        <div class="flex items-center space-x-3 mb-2">
                                            ...
                                            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold shadow-sm task-status-badge
                                    ...
                                    <div class="mt-6">
                                        <div class="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl p-6 border border-emerald-200 shadow-sm">
                                            <div class="flex items-center justify-between mb-4">
                                                ...
                                                @php
                                                    foreach($submissionTask->task->checklist_items as $indexItem => $item) {
                                                        if (isset($checklistProgress[$indexItem]) && $checklistProgress[$indexItem]) {
                                                            $completedCount++;
                                                        }
                                                    }
                                            ...
                                            <div class="space-y-3">
                                                @foreach($submissionTask->task->checklist_items as $indexItem => $item)
                                                    @php
                                                        $isChecked = isset($checklistProgress[$indexItem]) && $checklistProgress[$indexItem];
                                                    @endphp
                                    ...
                                    <div class="mt-6 manager-review-block">
                                        <div class="bg-gradient-to-r from-slate-50 to-gray-50 rounded-xl p-6 border border-slate-200 shadow-sm">
                                            ...
                                            @if($submissionTask->rejection_reason)
                                                <div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-4 rejection-reason-block">
                                            ...
                                            @if($submissionTask->manager_comment)
                                                <div class="bg-white bg-opacity-60 rounded-lg p-4 manager-comment-block">
                                ...
                                @else
                                    <!-- Lege container zodat we hier met JS manager review in kunnen injecten -->
                            ...
                            @if($submissionTask->status === 'completed')
                                <div class="ml-8 flex-shrink-0 w-80">
                                    <div class="bg-gradient-to-br from-white to-gray-50 rounded-xl p-6 border border-gray-200 shadow-sm review-action-box">
                            ...
                            @elseif($submissionTask->status === 'rejected')
                                <div class="ml-8 flex-shrink-0 w-80">
                                    <div class="bg-gradient-to-br from-red-50 to-rose-50 rounded-xl p-6 border border-red-200 shadow-sm review-action-box">
                            ...
                            @elseif($submissionTask->status === 'approved')
                                <div class="ml-8 flex-shrink-0 w-80">
                                    <div class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 border border-green-200 shadow-sm review-action-box">
                            ...
                            @elseif($submissionTask->status === 'redo_requested')
                                <div class="ml-8 flex-shrink-0 w-80">
                                    <div class="bg-gradient-to-br from-orange-50 to-amber-50 rounded-xl p-6 border border-orange-200 shadow-sm review-action-box">
                                        <div class="text-center">
...
function updateTaskUIAfterApprove(taskId, managerComment) {
    ...
    if (badge) {
        badge.className = 'inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold shadow-sm task-status-badge bg-green-100 text-green-800 border border-green-200';
        badge.innerHTML = `
            <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
            </svg>
    ...
    // Update rechter kaart
    const box = container.querySelector('.review-action-box');
    if (box) {
        box.className = 'bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 border border-green-200 shadow-sm review-action-box';
        box.innerHTML = `
            <div class="text-center">
                <div class="w-16 h-16 bg-green-500 rounded-full flex items-center justify-center mx-auto mb-4">
                    <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    // Manager review blok updaten / aanmaken
    if (managerComment && managerComment.trim() !== '') {
        ...
        if (!reviewBlock) {
            ...
            const proofSection = container.querySelector('.bg-gradient-to-r.from-purple-50.to-indigo-50')?.parentElement;
...
function updateTaskUIAfterReject(taskId, rejectionReason) {
    ...
    const badge = container.querySelector('.task-status-badge');
    if (badge) {
        badge.className = 'inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold shadow-sm task-status-badge bg-red-100 text-red-800 border border-red-200';
        badge.innerHTML = `
            <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/>
            </svg>
    ...
    const box = container.querySelector('.review-action-box');
    if (box) {
        box.className = 'bg-gradient-to-br from-red-50 to-rose-50 rounded-xl p-6 border border-red-200 shadow-sm review-action-box';
        box.innerHTML = `
            <div class="text-center mb-4">
                <div class="w-16 h-16 bg-red-500 rounded-full flex items-center justify-center mx-auto mb-4">
                    <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
