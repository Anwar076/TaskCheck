...
<div class="min-h-screen bg-gray-50 pt-6 sm:pt-8 pb-8 sm:pb-10 lg:pb-12">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if(empty($subscriptionLocked) && auth()->user()->isAdmin() && !auth()->user()->isSuperAdmin())
            <form method="POST" action="{{ route('dashboard.switch') }}" class="mb-6 xl:hidden">
                @csrf
                <input type="hidden" name="mode" value="admin">
                <button type="submit" class="flex w-full items-center justify-center gap-2 rounded-xl border border-blue-200 bg-white px-4 py-3 text-sm font-semibold text-blue-700 shadow-sm transition-colors hover:bg-blue-50">
        ...
        <form method="POST" action="{{ route('logout') }}" class="mt-8 xl:hidden">
            @csrf
            <button type="submit" class="flex w-full items-center justify-center gap-2 rounded-xl border border-red-200 bg-white px-4 py-3 text-sm font-semibold text-red-600 shadow-sm transition-colors hover:bg-red-50">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true">
